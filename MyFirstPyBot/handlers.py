...
def add_task(message, bot):
    ...
    db_execute("INSERT INTO Adds (ChatID, Task) VALUES (?, ?)", (message.chat.id, task_text))
    
    # Проверяем последний вставленный ID
    task_id = db_fetchall("SELECT last_insert_rowid()")[0][0]
